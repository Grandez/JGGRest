...
Our testing JGGRest application is implemented below:
...
At the moment JGGRest supports single backend - [Rserve](http://rforge.net/Rserve/).

Configurations:

- `JGGRest` can utilize all CPU cores and process requests in parallel. We will use multiple number of threads to see how it affects performance.
- During application `JGGRest` checks `RESTRSERVE_RUNTIME_ASSERTS` environment variable. It controls the amount of input validation `JGGRest` performs internally using [checkmate](https://cran.r-project.org/web/packages/checkmate/index.html) package. Despite the fact that runtime checks comes with additional time overhead this variable is set to `TRUE` by default. We value correctness and robustness of the application at the first place. We will benchmark application with different values of `RESTRSERVE_RUNTIME_ASSERTS` to see the difference.
...
apib_bench = function(
  ...
  for (fib_count in fib_counts) {
    for (runtime_check in runtime_checks) {
      ...
      # Wait for R to start
      Sys.sleep(2)
      flavor = if (runtime_check) "JGGRest + runtime checks" else "JGGRest"
      results = c(
        results, 
        run_apib(
          n_sec = n_sec, 
...
```
<img src="https://github.com/rexyai/JGGRest/blob/master/vignettes/img/bench-rps.png?raw=true" width="640"/>


## No keep-alive

Keep in mind that creating new connections is quite expensive for any HTTP server. For `JGGRest`'s `Rserve` backend this is particularly true since for each new connection it forks a child process (which has relatively high cost). With other backends slow down might be less significant. 
...
plot_results(
  ...
  "JGGRest - No keep-alive", 
  "../img/bench-rps-no-keep-alive.png"
)
```
<img src="https://github.com/rexyai/JGGRest/blob/master/vignettes/img/bench-rps-no-keep-alive.png?raw=true" width="640"/>
...
maintain pool of connections to JGGRest and hence won't suffer from creating new connections.
...
```
<img src="https://github.com/rexyai/JGGRest/blob/master/vignettes/img/bench-rps-vs-plumber.png?raw=true" width="640"/>

As can be seen `JGGRest` performs very well on every workload and scales linearly with number of cores. 
...
<img src="https://github.com/rexyai/JGGRest/blob/master/vignettes/img/bench-rps-vs-plumber-no-keepalive.png?raw=true" width="640"/>

Due to the overhead of creating a new process for each request and [R's byte compiler overhead](https://github.com/rexyai/JGGRest/issues/149) `JGGRest` with `Rserve` backend does not perform as quickly as `plumber` when computing instantaneous routes.  However, `JGGRest` still shows it's strength when executing routes that have high computational costs. No extra coding logic is needed to leverage `JGGRest`'s multi-threaded execution. Mixing `plumber` and `future` together for _high computation_ routes brings performance that scales with the number of concurrent requests, but at the cost of extra route logic and domain knowledge.
