...
`JGGRest` comes with several build-in middlewares (`AuthMiddleware`, `CORSMiddleware`, `ETagMiddleware`) and generic `Middleware` class which facilitates user to create a custom middleware. 
...
app$add_get("/foo", function(.req, .res) {
  body = JGGRest::to_json(.req$parameters_query)
  .res$set_body(body)
...
Assume you would like to analyze how your web service works. For that you may need log every request and response in order to see whether service replies with errors and what can cause these errors. This is a perfect task for a middleware and here is how you can achieve this with `JGGRest`:
