#' @title Creates application - JGGRest usage starts from here
#'
#' @description
...
#' processed:
#' - `options("JGGRest.headers.server")` controls response `"Server"` header
#' - `options("JGGRest.headers.split")` controls which header values
...
#' See `options("JGGRest.runtime.asserts")`
#' @export
#'
...
#' desc_file = system.file("DESCRIPTION", package = "JGGRest")
#' # add route
...
Application = R6::R6Class(
  ...
  public = list(
    ...
    #'   his own class (see `JGGRest:::HTTPErrorFactory`). However we believe
    ...
    #'   passed further to JGGRest execution pipeline.
  ...
  private = list(
    ...
    eval_with_error_handling = function(expr) {
      expanded_traceback = isTRUE(getOption("JGGRest.runtime.traceback", TRUE))
