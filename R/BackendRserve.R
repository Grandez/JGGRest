...
BackendRserve = R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(..., jit_level = 0L, precompile = FALSE) {
      ...
      headers_to_split = getOption("JGGRest.headers.split", character())
      private$headers_to_split = checkmate::assert_character(headers_to_split)
      invisible(self)
    },
    #' @description
    #' Starts JGGRest application from current R session.
    ...
    start = function(app, http_port = 8080, ..., background = FALSE) { # nocov start
      ...
      if (interactive()) {
        ...
        if (identical(.Platform[['GUI']], "RStudio")) {
          msg = "Starting JGGRest app which uses Rserve backend from within"
          ...
          warn_msg = "Starting JGGRest app from interactive session"
      ...
      if (is.null(ARGS[["port"]])) {
        ...
        ARGS[["port"]] = JGGRest:::find_port()
      }
      if (JGGRest:::port_is_taken(ARGS[["port"]])) {
      ...
      .GlobalEnv[[".http.request"]] = function(url, parameters_query, body, headers) {
        ...
        # see https://github.com/rexyai/JGGRest/issues/158 for the hints
        # con = nullfile()
      ...
      # see https://github.com/rexyai/JGGRest/issues/149
    ...
    set_request = function(request, path = "/", parameters_query = NULL, headers = NULL, body = NULL) {
      ...
      if (isTRUE(getOption('JGGRest.runtime.asserts', TRUE))) {
