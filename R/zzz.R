...
#' @useDynLib JGGRest, .registration=TRUE
...
.onUnload = function(libpath) { # nocov start
  library.dynam.unload("JGGRest", libpath)
} # nocov end

# https://en.wikipedia.org/wiki/List_of_HTTP_header_fields
# https://stackoverflow.com/a/29550711/3048453
JGGRest.headers.split = c(
  "accept",
  "accept-charset",
...
.onLoad = function(...) { # nocov start
  ...
  runtime_asserts = Sys.getenv("JGGRest_RUNTIME_ASSERTS", unset = TRUE)
  runtime_asserts = isTRUE(as.logical(runtime_asserts))
  JGGRest_options = list(
    "JGGRest.runtime.asserts" = runtime_asserts,
    "JGGRest.headers.server" = paste(
      paste("JGGRest", packageVersion("JGGRest"), sep = "/"),
      paste("Rserve", packageVersion("Rserve"), sep = "/"),
      sep='; '
    ),
    "JGGRest.headers.split" = JGGRest.headers.split
  )

  toset = !(names(JGGRest_options) %in% names(default_options))
  if (any(toset)) options(JGGRest_options[toset])
...
#' @rdname IDE-hints
#' @export
.req = NULL # see zzz.R on how JGGRest initializes this object during .onLoad

#' @rdname IDE-hints
#' @export
.res = NULL # see zzz.R on how JGGRest initializes this object during .onLoad
