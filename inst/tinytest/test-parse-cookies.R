...
# import functions
cpp_parse_cookies = JGGRest:::cpp_parse_cookies

# Empty input
expect_error(cpp_parse_cookies(NULL))
expect_true(inherits(cpp_parse_cookies(NA), "list"))
expect_equal(length(cpp_parse_cookies(NA)), 0L)
expect_true(inherits(cpp_parse_cookies(NA_character_), "list"))
expect_equal(length(cpp_parse_cookies(NA_character_)), 0L)
expect_true(inherits(cpp_parse_cookies(""), "list"))
expect_equal(length(cpp_parse_cookies("")), 0L)
...
parsed = cpp_parse_cookies(cookie)
